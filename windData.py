...
def clean_data(name_list_string):
    ...
    for name_index in range(len(name_list_string)):
        ...
        #flagging coordinants over water
...
def coordinant_list():
    coordinants = []
    for i in range(25): #for each latitude
        for j in range(59): #for each longitude
            if water_binary_strings[-115-i][55+j] == '1':
                coordinants.append((25+i,55+j))
    return coordinants

#print(coordinant_list())
#print(len(coordinant_list()))
...
def best_angle_energy(u_array,v_array,num_angles,width):
    ...
    width: width in radians that wind can be collected with no loss
    Outputs:
    angle_matrix: angle with highest AEP (radians) for each coordinant
    best_energy: amount of energy (J) produced at each best angle
    """
    #power at all angles for all coordinants
    ...
    #creating an array of best angle per coordinant
    angle_matrix = []
...
def coordinant_info(u_array,v_array,num_angles,width):
    """Inputs:
    u_array and v_array: wind vectors in the u and v directions in m/s
    angle: angle that the wind turbine collects wind from
    width: width in radians that wind can be collected with no loss
...
def all_generators_energy(u_array,v_array,num_angles,width,num_gen):
    info = coordinant_info(u_array,v_array,num_angles,width)
    power_array = info[1][0] #power collected at optimal angle
    original_energy = info[0][1]
    energy_list = []
    for i in range(num_gen): #number of dif sized generators
        #max wind power that can be collected
        max = power_function(7+(i/num_gen)*6) #max wind speeds between 7 and 12 m/s
    ...
    for i in range(len(percent_all_gen[0,:,0])): #latitude
        ...
        for j in range(len(percent_all_gen[0,0,:])): #longitude
            for k in range(num_gen): #generator size
                above_best = True #if all percentages are above best percentage
...
def weibull_distribution(u_array,v_array,num_angles,width):
    speed_array = coordinant_info(u_array,v_array,num_angles,width)[1][2]
...
info = coordinant_info(u10,v10,16,math.pi/2)
